...
1. Lejupielādējiet `gurucli-v1.8.5-mac`.
2. `cd ~/Downloads`
3. Padariet izpildāmu: `chmod +x gurucli-v1.8.5-mac`
...
2. Padariet izpildāmu: `chmod +x GuruShotsAutoVote-v1.8.5-*.AppImage` (vai failu pārvaldniekā → Properties → Permissions).
3. Palaidiet: `./GuruShotsAutoVote-v1.8.5-*.AppImage`

**CLI:**

1. Lejupielādējiet `gurucli-v1.8.5-linux` (vai `-linux-arm`).
2. `cd ~/Downloads`
...
1. Telefonā atveriet [jaunākā izlaiduma lapu](https://github.com/isthisgitlab/gurushots-auto-vote/releases/latest) un piesitiet `GuruShotsAutoVote-v1.8.5.apk`.
...
```bash
./gurucli-v1.8.5-[platforma] set-global-default exposure 80
./gurucli-v1.8.5-[platforma] set-setting onlyBoost true --challenge=12345
./gurucli-v1.8.5-[platforma] list-settings --challenge=12345
./gurucli-v1.8.5-[platforma] logs --error --lines=50
```
...
- **Noklusējuma secība.** **Kārtot pēc noklusējuma secības** vispirms liek noteikumus ar nosaukumu (ir tieši pirms sākas ar pirms satur, garāki nosaukumi vispirms), tad bilžu skaits + ilgums, bilžu skaits, ilgums, veids un tags — piem., `4 bildes + ≥ 168 h` virs `4 bildes` virs `≥ 168 h`. Secību var brīvi mainīt ar bultiņām; saglabājot tā tiek saglabāta.
