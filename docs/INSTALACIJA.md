...
2. **Padarīt izpildāmu**: Labais klikšķis uz faila → Properties → Permissions → Atzīmējiet "Allow executing file as program"
   - Vai izmantojiet termināli: `chmod +x GuruShotsAutoVote-v0.3.0-beta.6-*.AppImage`
3. **Palaist**: Veiciet dubultklikšķi uz faila vai palaidiet no termināļa: `./GuruShotsAutoVote-v0.3.0-beta.6-*.AppImage`
...
1. Izveidojiet `.desktop` failu:
   ...
   Exec=/pilns/ceļš/uz/GuruShotsAutoVote-v0.3.0-beta.6-x86_64.AppImage
   Icon=
   Type=Application
   Categories=Utility;
   EOL
