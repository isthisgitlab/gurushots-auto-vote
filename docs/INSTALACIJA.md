...
1. Lejupielādējiet `gurucli-v1.4.1-mac`.
2. `cd ~/Downloads`
3. Padariet izpildāmu: `chmod +x gurucli-v1.4.1-mac`
4. Notīriet karantīnas karogu (tikai pārlūka lejupielādēm): `xattr -d com.apple.quarantine ./gurucli-v1.4.1-mac`
5. Palaidiet: `./gurucli-v1.4.1-mac help`
...
2. Padariet izpildāmu: `chmod +x GuruShotsAutoVote-v1.4.1-*.AppImage` (vai failu pārvaldniekā → Properties → Permissions).
3. Palaidiet: `./GuruShotsAutoVote-v1.4.1-*.AppImage`

**CLI:**

1. Lejupielādējiet `gurucli-v1.4.1-linux` (vai `-linux-arm`).
2. `cd ~/Downloads`
...
1. Telefonā atveriet [jaunākā izlaiduma lapu](https://github.com/isthisgitlab/gurushots-auto-vote/releases/latest) un piesitiet `GuruShotsAutoVote-v1.4.1.apk`.
...
```bash
./gurucli-v1.4.1-[platforma] login    # autentificējieties vienreiz (saglabā tokenu)
./gurucli-v1.4.1-[platforma] run      # viens pilns auto-stratēģijas cikls (boost/turbo/aizpilde/slieksnis-balsošana)
./gurucli-v1.4.1-[platforma] start    # nepārtraukta balsošana (Ctrl+C, lai apturētu)
```
...
```bash
./gurucli-v1.4.1-[platforma] set-global-default exposure 80
./gurucli-v1.4.1-[platforma] set-setting onlyBoost true --challenge=12345
./gurucli-v1.4.1-[platforma] list-settings --challenge=12345
./gurucli-v1.4.1-[platforma] logs --error --lines=50
```
