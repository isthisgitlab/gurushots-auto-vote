...
| **Linux (x64)**   | [📥 GuruShotsAutoVote-v1.0.0-x86_64.AppImage](https://github.com/isthisgitlab/gurushots-auto-vote/releases/latest/download/GuruShotsAutoVote-v1.0.0-x86_64.AppImage) | ~50 MB | AppImage            |
| **Linux (ARM64)** | [📥 GuruShotsAutoVote-v1.0.0-arm64.AppImage](https://github.com/isthisgitlab/gurushots-auto-vote/releases/latest/download/GuruShotsAutoVote-v1.0.0-arm64.AppImage)   | ~50 MB | AppImage            |
...
1. Download `gurucli-v1.0.0-mac`.
2. `cd ~/Downloads`
3. Make it executable: `chmod +x gurucli-v1.0.0-mac`
4. Clear the quarantine flag (browser downloads only): `xattr -d com.apple.quarantine ./gurucli-v1.0.0-mac`
5. Run: `./gurucli-v1.0.0-mac help`
...
2. Make it executable: `chmod +x GuruShotsAutoVote-v1.0.0-*.AppImage` (or via file-manager → Properties → Permissions).
3. Run it: `./GuruShotsAutoVote-v1.0.0-*.AppImage`

**CLI:**

1. Download `gurucli-v1.0.0-linux` (or `-linux-arm`).
2. `cd ~/Downloads`
3. `chmod +x gurucli-v1.0.0-linux`
4. `./gurucli-v1.0.0-linux help`

#### 📱 Android (sideload)

The Android build is **not on Google Play** — install via direct APK download.

1. On the phone, open the [latest release page](https://github.com/isthisgitlab/gurushots-auto-vote/releases/latest) and tap `GuruShotsAutoVote-v1.0.0.apk`.
...
```bash
./gurucli-v1.0.0-[platform] login    # authenticate once (saves a token)
./gurucli-v1.0.0-[platform] run      # one full auto-strategy cycle (boost/turbo/fill/threshold-aware vote)
./gurucli-v1.0.0-[platform] start    # continuous voting (Ctrl+C to stop)
```
...
```bash
./gurucli-v1.0.0-[platform] set-global-default exposure 80
./gurucli-v1.0.0-[platform] set-setting onlyBoost true --challenge=12345
./gurucli-v1.0.0-[platform] list-settings --challenge=12345
./gurucli-v1.0.0-[platform] logs --error --lines=50
```
