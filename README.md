...
1. Download `gurucli-v1.8.5-mac`.
2. `cd ~/Downloads`
3. Make it executable: `chmod +x gurucli-v1.8.5-mac`
...
2. Make it executable: `chmod +x GuruShotsAutoVote-v1.8.5-*.AppImage` (or via file-manager → Properties → Permissions).
3. Run it: `./GuruShotsAutoVote-v1.8.5-*.AppImage`

**CLI:**

1. Download `gurucli-v1.8.5-linux` (or `-linux-arm`).
2. `cd ~/Downloads`
...
1. On the phone, open the [latest release page](https://github.com/isthisgitlab/gurushots-auto-vote/releases/latest) and tap `GuruShotsAutoVote-v1.8.5.apk`.
...
```bash
./gurucli-v1.8.5-[platform] set-global-default exposure 80
./gurucli-v1.8.5-[platform] set-setting onlyBoost true --challenge=12345
./gurucli-v1.8.5-[platform] list-settings --challenge=12345
./gurucli-v1.8.5-[platform] logs --error --lines=50
```
...
GuruShots recycles each challenge under a fresh ID every rotation, so a per-challenge override is lost when the challenge comes back. **Challenge rules** match on what survives a rotation instead. They are managed in the GUI Settings modal under **Challenge Rules** (GUI only; the CLI and Android apply them).
...
- **Default order.** **Sort by default order** puts rules naming a title first (exact before starts-with before contains, longer titles first), then photo count + length, photo count, length, type, and tag — e.g. `4 photos + ≥ 168 h` above `4 photos` above `≥ 168 h`. Reorder freely with the arrows; saving keeps your order.
...
- **Upgrading.** Saved rules are put into the default order once, and the former "join timing by category" rules are moved into this list below them. Because a lower rule now fills in what a higher one leaves empty, the settings log gets a warning for each pair of rules where that could newly switch auto-join or auto-submit on — review the order if you see one.
