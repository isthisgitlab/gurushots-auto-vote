...
- **Type checking** (`pnpm typecheck` → `ttsc --noEmit`): this is a JavaScript project — TypeScript is a **checker only, never a compiler** (`tsconfig.json` sets `noEmit` + `checkJs: false`). Only files with a `// @ts-check` comment are type-checked, so add JSDoc + `// @ts-check` to the shared core incrementally. Opted in: `apiFactory.js`, `format/photoUrl.js`, `settings/schema.js`, `settings/uiDefaults.js`, `services/VotingLogic.js`, `services/challengeTitlePin.js`, `services/deadlineNotifications.js`, `services/newEntryTracker.js`, `voting/currencyActions.js`, `voting/currencyAuto.js`, `voting/entrySlot.js` (`grep -rl '^// @ts-check' src/js` for the live list). CI gates on it. When a `// @ts-check` file imports an un-typed module whose inferred signature is too narrow (e.g. a `= null` default), cast the import to `any` at the boundary until that module is typed too.
