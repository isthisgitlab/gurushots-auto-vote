...
- **API surface swap**: `src/js/apiFactory.js` selects real vs mock implementations at runtime based on `settings.mock`. All callers go through the factory — do not import `src/js/api/*`, `src/js/strategies/*` or `src/js/mock/*` directly from business logic. `src/js/api/` is the transport layer (api-client + endpoint wrappers, no `services/` imports); the real-mode composition over it (voting pass with join/claim pre-steps, entry-picking boost, title-pinned challenge read) lives in `src/js/strategies/real/`, the mock counterpart in `src/js/mock/` (`strategy.js` over the `endpoints/*` fakes, assembled into `mockApiClient` behind the `mock/index.js` facade).
...
- **Settings Architecture**: Settings facade lives at `src/js/settings.js`. Schema (keys + defaults + validation) is in `src/js/settings/schema.js`; persistence transport (fs on Electron/CLI, `@capacitor/preferences` on Android) is in `src/js/settings/storage.js`. The facade's internals (load/merge/migrate/save in `settings/persistence.js` + `settings/migrations.js`, overrides, title rules, profiles, title pins, reset) live in sibling modules under `src/js/settings/` and are private to it — ESLint rejects requiring them from outside that directory. Always go through the facade.
...
> Package manager: **pnpm 12+** (pinned via `packageManager` in `package.json`, bootstrap with `corepack enable`). Use `pnpm <script>` everywhere — the `npm` CLI is not supported.
...
- **Build**: `pnpm build:mac` · `build:win` · `build:linux` · `build:android` · `build:cli:all`. Each has a `:lite` twin (`build:mac:lite`, `build:cli:mac:lite`, `build:android:lite`, …) that leaves the local vision model and its runtime out — see `docs/architecture.md` §4a.
- **CLI build prereq**: needs `tar` and network access on first build (the pinned SigLIP model is fetched into `.cache/vision-model` by `scripts/fetch-vision-model.js`), plus, on macOS, `codesign` (Xcode CLT). The binary is deliberately NOT stripped: the embedded vision runtime's native addons (onnxruntime-node, sharp) need Node's exported N-API symbols. Each CLI target is built on its native-arch runner — mac on `macos-26`, linux x64 on `ubuntu-latest`, linux arm64 on `ubuntu-24.04-arm` (separate `build-cli-arm` job). Local cross-arch builds (e.g. `build:cli:linux-arm` from an x64 host) aren't supported by the script — run on matching hardware or let CI handle it. UPX is intentionally NOT used — macOS AMFI rejects packed Mach-O, and postject's ELF section injection trips UPX's `bad e_phoff` structural check on Linux. Mac CLI binary is ad-hoc signed by the build — no Developer ID / notarization, so browser-downloaded copies still hit Gatekeeper and need `xattr -d com.apple.quarantine`.
...
- **Code hygiene**: `pnpm knip` (unused files/deps/types — config in `knip.json`; the `exports`/`nsExports` rules are off because they misfire on this codebase's CJS namespace-access pattern) · `pnpm size` (renderer bundle brotli **delta** gate — `scripts/check-size-delta.js` compares each bundle's brotli size against committed `.size-baseline.json` and fails only when one grows more than a threshold (default 5 kB, override `SIZE_DELTA_KB`) since the baseline; it catches an accidental heavy import — e.g. something newly reachable from `App.jsx` pulling in `settings/schema.js`, whose CJS `require('zod')` adds ~60 kB brotlied and can't be tree-shaken, which is why size-relevant bounds live in the dependency-free `settings/limits.js` — while letting a few kB of real feature growth through. On intentional growth run `pnpm size:update` and commit the refreshed baseline) · `pnpm size:cli` (coarse SEA binary / `cli-bundled.js` size guard, `scripts/check-cli-size.js`)
...
- **Build-script approval**: pnpm 12 requires opting each native postinstall script into `allowBuilds:` in `pnpm-workspace.yaml`. Keep that list minimal — every entry is an arbitrary-code-execution opportunity.
- **Type checking** (`pnpm typecheck` → `ttsc --noEmit`): this is a JavaScript project — TypeScript is a **checker only, never a compiler** (`tsconfig.json` sets `noEmit` + `checkJs: false`). Only files with a `// @ts-check` comment are type-checked, so add JSDoc + `// @ts-check` to the shared core incrementally. Opted in: `apiFactory.js`, `format/photoUrl.js`, `settings/schema.js`, `settings/uiDefaults.js`, `services/VotingLogic.js` and every module behind it in `services/decisions/`, `services/challengeTitlePin.js`, `services/deadlineNotifications.js`, `services/newEntryTracker.js`, `voting/currencyActions.js`, `voting/currencyAuto.js`, `voting/entrySlot.js` (`grep -rl '^// @ts-check' src/js` for the live list). CI gates on it. When a `// @ts-check` file imports an un-typed module whose inferred signature is too narrow (e.g. a `= null` default), cast the import to `any` at the boundary until that module is typed too.
- **ttsc toolchain** (`ttsc` + `@ttsc/lint`, exact-pinned in lockstep — samchon/ttsc, typescript-go based): `ttsc` replaces `tsc` in the `typecheck` script and bundles its own typescript-go build matching the pinned `typescript` version; `tsc` stays installed for `typecheck:tsc` (parity cross-check after any ttsc bump; rollback is manual — point the `typecheck` script back at `tsc --noEmit` and reconcile the lint-config/knip/docs state, there is no automatic fallback). The same `ttsc --noEmit` pass runs the **@ttsc/lint** rules from `lint.config.ts` — diagnostic codes in the **TS9000–TS17999 band are lint rules** (each prints its `[rule-name]`), everything else is a compiler type error; error-severity rules fail the gate, warnings don't. **Lint scope ≠ type-check scope**: the `// @ts-check` opt-in gates type-error reporting only, while every `lint.config.ts` rule (including the type-aware `typescript/*` ones, which use the checker's inferred types) runs over the **whole** tsconfig program (`src/js/**/*.js`) — a file is never exempt from the lint gate just because it hasn't opted into type-checking. ESLint + Prettier keep owning `.jsx`, `scripts/`, tests, and all formatting; the `format` block must stay absent so ttsc never fights Prettier. Supply-chain note: these are devDependencies, so the blocking `pnpm audit --prod` gate does NOT cover them (a non-blocking full audit step in the test workflow surfaces dev-tree CVEs); Dependabot bumps `ttsc` and `@ttsc/lint` together (the `ttsc-toolchain` group in `.github/dependabot.yml`) because `@ttsc/lint` has no peer pin — review the release diff on every bump.
- **Repository analysis**: Ripwire is the supported structural-analysis backend for the shared core, `.jsx`, tests, scripts, and whole-repository queries. Use its MCP tools or the pinned `ripwire` CLI workflow appropriate to the question; use `ttsc`/`tsc` for compiler-exact type diagnostics.
...
- **CI hygiene gates**: the test workflow runs `pnpm size` (renderer bundle budgets) on the main `ubuntu-slim` job and `pnpm knip` (dead code / unused deps) in a separate `knip` job on `ubuntu-latest` — knip's oxc parser allocates a ~2 GiB ArrayBuffer that the slim runner can't satisfy. `pnpm typecheck` likewise runs in its own `typecheck` job on `ubuntu-latest`: a cold ttsc cache compiles the `@ttsc/lint` Go plugin from source (~25 min on one core), which the slim runner's job cap cancels; the job caches only `node_modules/.cache/ttsc/plugins` (~35 MB, keyed on `pnpm-lock.yaml`), which is all a warm start needs. Each CLI build job runs `pnpm size:cli`.
